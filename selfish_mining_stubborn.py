...
# select false and false to do trail-stubborn, true and false to lead-stubborn, true and false to equal-fork-stubborn
trail_stubborn = 3
lead_stubborn = False
            ...
            if r <= alpha:
                ...
                if private_lead > 0:
                    ...
                    if bad:
                        ...
                        other_height = len(private_mining_times)
                        current_t = arrival_t
        ...
        if state == -1:
            if r <= alpha:
                if equal_fork_stubborn:
                    ...
                    num_selfish_blocks += private_lead + 2
                    attacker_reorg_sizes[1] += 1
                    longest_chain_length += private_lead + 2
            elif r <= alpha + (1 - alpha) * gamma:
                num_selfish_blocks += private_lead + 1
                attacker_reorg_sizes[1] += 1
                longest_chain_length += private_lead + 2
            else:
                if trail_stubborn > 0:
                    private_mining_times = [released_time] + private_mining_times
                    private_lead = -1 + private_lead
                    honest_branch_size = 2
                    is_trailing = True
                    continue
                else:
                    honest_reorg_sizes[private_lead + 1] += 1
            ...
            if r <= alpha:
                ...
                if with_rule:
                    if ts_honest > local_t + FUTURE_LIMIT or ts_honest < local_t - PAST_LIMIT:
                        bad = True
                if bad:
                    reject_until = max(arrival_t + TIMEOUT, ts_honest - FUTURE_LIMIT)
                    honest_height = 1
                    other_height = 0
                    current_t = arrival_t
                    while current_t < reject_until:
...
for _ in range(num_runs):
  without_rule, a_sizes, h_sizes = simulate(alpha, gamma, sim_time, with_rule=False)
  without_sum += without_rule
  without_attacker_sizes += a_sizes
  without_honest_sizes += h_sizes
  with_rule, a_sizes, h_sizes = simulate(alpha, gamma, sim_time, with_rule=True)
  with_sum += with_rule
